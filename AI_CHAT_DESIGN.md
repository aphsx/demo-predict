...
TABLE: prediction_batches
  ...
  batch_name TEXT                 -- ชื่อ batch เช่น "บริษัท A มีนาคม 2026"
  status TEXT                     -- 'ready' | 'processing' | 'error'
  uploaded_at TIMESTAMPTZ         -- วันที่อัพโหลด
...
## 8. การจัดการ Context สำหรับหลาย Batch

ผู้ใช้เลือก batch ก่อนเริ่มคุย → `batch_id` ถูกฝังใน context ตลอด session
...
class ChatSession:
    ...
    # batch_id ถูก inject เข้า SQL อัตโนมัติ — ผู้ใช้ไม่ต้องพิมพ์เอง
...
```
[หน้า Batch List]
  ○ บริษัท A - มีนาคม 2026 (10,000 คน) [เลือก]
  ○ บริษัท B - กุมภาพันธ์ 2026 (5,200 คน)
  ○ ทดสอบ - มกราคม 2026 (3,000 คน)

[คลิก "เลือก" → เข้า Chat ที่ผูกกับ batch นั้น]
```
...
| Context | batch_id ใน session | isolate ข้อมูลแต่ละ batch |
