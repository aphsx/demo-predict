...
PRIORITY_WEIGHTS = {"risk": 50.0, "value": 30.0, "credit": 20.0}
TOPUP_CAP_DAYS = 365
INSERT_CHUNK = 1000
...
def _apply_derived(frame: pd.DataFrame, cutoff: pd.Timestamp) -> pd.DataFrame:
    ...
    # priority score (§5.2)
    p_risk = churn_prob.fillna(0.0)
    value_rank = pd.Series(0.0, index=frame.index)
    if pool.any():
        value_rank.loc[pool] = clv[pool].rank(pct=True)
    p_credit = pd.Series(
        np.where(days_series.notna(), np.maximum(0.0, 1.0 - days_series / 90.0), 0.0),
        index=frame.index,
    )
    frame["priority_score"] = (
        PRIORITY_WEIGHTS["risk"] * p_risk
        + PRIORITY_WEIGHTS["value"] * value_rank
        + PRIORITY_WEIGHTS["credit"] * p_credit
    ).round(2)

    frame["priority_reason"] = [
        _priority_reason(risk, value, credit, prob, clv_value, day_value)
        for risk, value, credit, prob, clv_value, day_value in zip(
            PRIORITY_WEIGHTS["risk"] * p_risk,
            PRIORITY_WEIGHTS["value"] * value_rank,
            PRIORITY_WEIGHTS["credit"] * p_credit,
            churn_prob,
    ...
    # recommended action + follow-up (§5.3)
    actions, followups = [], []
    for _, row in frame.iterrows():
        action, followup = _recommend_action(row, cutoff)
        actions.append(action)
        followups.append(followup)
    frame["recommended_action"] = actions
    frame["recommended_followup_date"] = followups
    return frame
...
def _priority_reason(
    ...
    churn_prob: float,
    clv_value: float,
    days_until_topup: float,
) -> str:
    dominant = max(
        ("risk", risk_component), ("value", value_component), ("credit", credit_component),
        key=lambda pair: pair[1],
    )[0]
    if dominant == "risk" and not pd.isna(churn_prob):
        clv_text = f" × CLV ฿{clv_value:,.0f}" if not pd.isna(clv_value) and clv_value > 0 else ""
        return f"เสี่ยง churn {churn_prob * 100:.0f}%{clv_text}"
    if dominant == "value" and not pd.isna(clv_value) and clv_value > 0:
        return f"ลูกค้ามูลค่าสูง — CLV คาดการณ์ ฿{clv_value:,.0f}"
    if dominant == "credit" and not pd.isna(days_until_topup):
        return f"เครดิตจะหมดในราว {int(days_until_topup)} วัน"
    return "ความเสี่ยงรวมต่ำ — เฝ้าดูตามรอบ"


def _recommend_action(row: pd.Series, cutoff: pd.Timestamp) -> tuple[str, str | None]:
    risk = row["churn_risk_level"]
    tier = row["customer_value_tier"]
    urgency = row["credit_urgency_level"]
    stage = row["lifecycle_stage"]
    sub_stage = row["sub_stage"]
    days_since = row.get("days_since_last_activity")

    def date_offset(days: int) -> str:
        return str((cutoff + pd.Timedelta(days=days)).date())

    if risk in ("high", "critical") and tier in ("high", "mid"):
        return "save_call", date_offset(3)
    if urgency == "critical":
        return "topup_reminder", date_offset(1)
    if urgency == "warning":
        return "topup_reminder", date_offset(7)
    if risk in ("high", "critical"):
        return "retention_campaign", date_offset(14)
    if stage == "Active Free" and (row["usage_trend"] == "increasing" or tier == "high"):
        return "upsell_offer", date_offset(7)
    if sub_stage == "Churned Paid" and days_since is not None and not pd.isna(days_since) and days_since <= 270:
        return "winback_manual", date_offset(30)
    return "monitor", None
